`imputationSummary` <- function(
  ...
  IMP_N <- M.IMP <- Mean_SGPB_Imputed <- Mean_SGPB_Observed <- Mean_SGP_Imputed <-
  Mean_SGP_Observed <- Mean_SS_Imputed <- Mean_SS_Observed <- N <- SCALE_SCORE_OBSERVED <-
  SGPB_ADF <- SGPB_B <- SGPB_CI_high <- SGPB_CI_high_simp <- SGPB_CI_low <-
  SGPB_CI_low_simp <- SGPB_F_Simp <- SGPB_F_Stat <- SGPB_F_p <- SGPB_F_p_simp <-
  SGPB_Gamma <- SGPB_Lamda <- SGPB_RIV <- SGPB_T <- SGPB_U_Imp <- SGPB_U_bar <-
  SGPB_tSimp <- SGPB_tStat <- SGP_ADF <- SGP_B <- SGP_BASELINE_OBSERVED <- SGP_CI_high <-
  SGP_CI_high_simp <- SGP_CI_low <- SGP_CI_low_simp <- SGP_F_Simp <- SGP_F_Stat <-
  SGP_F_p <- SGP_F_p_simp <- SGP_Gamma <- SGP_Lamda <- SGP_OBSERVED <- SGP_RIV <-
  SGP_T <- SGP_U_Imp <- SGP_U_bar <- SGP_tSimp <- SGP_tStat <- SS_ADF <- SS_B <-
  SS_CI_high <- SS_CI_high_simp <- SS_CI_low <- SS_CI_low_simp <- SS_F_Simp <-
  SS_F_Stat <- SS_F_p <- SS_F_p_simp <- SS_Gamma <- SS_Lamda <- SS_RIV <- SS_T <-
  SS_U_Imp <- SS_U_bar <- SS_tSimp <- SS_tStat <- NULL
  ...
  smry_obs <- data.to.summarize[, list(
        ...
        SE_SS_Observed = se(SCALE_SCORE_OBSERVED),
        Mean_SGP_Observed = mean(SGP_OBSERVED, na.rm = TRUE),
        SE_SGP_Observed = se(SGP_OBSERVED),
        Mean_SGPB_Observed = mean(SGP_BASELINE_OBSERVED, na.rm = TRUE),
        SE_SGPB_Observed = se(SGP_BASELINE_OBSERVED),
  ...
  ##    T - Total Variance
  smry_all[, SS_T := SS_U_bar + (1 + 1/MM)*SS_B]
  smry_all[, SGP_T := SGP_U_bar + (1 + 1/MM)*SGP_B]
  smry_all[, SGPB_T := SGPB_U_bar + (1 + 1/MM)*SGPB_B]

  ###   Variance Ratios :: 2.3.5

  ##    Lambda -- proportion of variation attributed to the missing data. eq 2.24
  smry_all[, SS_Lamda := ((SS_B + SS_B/MM)/SS_T)]
  smry_all[, SGP_Lamda := ((SGP_B + SGP_B/MM)/SGP_T)]
  smry_all[, SGPB_Lamda := ((SGPB_B + SGPB_B/MM)/SGPB_T)]

  ##    RIV (r) -- relative increase in variance due to nonresponse. eq 2.25
  smry_all[, SS_RIV := (SS_Lamda/(1-SS_Lamda))]
  smry_all[, SGP_RIV := (SGP_Lamda/(1-SGP_Lamda))]
  smry_all[, SGPB_RIV := (SGPB_Lamda/(1-SGPB_Lamda))]
  ...
  smry_all[, SS_Gamma := (SS_RIV+(2/(SS_ADF+3)))/(1+SS_RIV)]
  smry_all[, SGP_Gamma := (SGP_RIV+(2/(SGP_ADF+3)))/(1+SGP_RIV)]
  smry_all[, SGPB_Gamma := (SGP_RIV+(2/(SGPB_ADF+3)))/(1+SGPB_RIV)]
  ...
  smry_all[!is.na(SS_B) & SS_B != 0, SS_tSimp := qtSimp*(sqrt((1 + 1/MM)*SS_B))]
  smry_all[!is.na(SGP_B) & SGP_B != 0, SGP_tSimp := qtSimp*(sqrt((1 + 1/MM)*SGP_B))]
  smry_all[!is.na(SGPB_B) & SGPB_B != 0, SGPB_tSimp := qtSimp*(sqrt((1 + 1/MM)*SGPB_B))]
  ...
  smry_all[!is.na(SS_ADF) & SS_ADF != 0, SS_F_p := pf(SS_F_Stat, df1=1, df2=SS_ADF, lower.tail=FALSE)]
  smry_all[, SS_F_Simp := ((Mean_SS_Observed - Mean_SS_Imputed)^2)/((1 + 1/MM)*SS_B)]
  smry_all[, SS_F_p_simp := pf(SS_F_Simp, df1=1, df2=nu, lower.tail=FALSE)]

  smry_all[, SGP_F_Stat := ((Mean_SGP_Observed - Mean_SGP_Imputed)^2)/SGP_T]
  smry_all[!is.na(SGP_ADF) & SGP_ADF != 0, SGP_F_p := pf(SGP_F_Stat, df1=1, df2=SGP_ADF, lower.tail=FALSE)]
  smry_all[, SGP_F_Simp := ((Mean_SGP_Observed - Mean_SGP_Imputed)^2)/((1 + 1/MM)*SGP_B)]
  smry_all[, SGP_F_p_simp := pf(SGP_F_Simp, df1=1, df2=nu, lower.tail=FALSE)]

  smry_all[, SGPB_F_Stat := ((Mean_SGPB_Observed - Mean_SGPB_Imputed)^2)/SGPB_T]
  smry_all[!is.na(SGPB_ADF) & SGPB_ADF != 0, SGPB_F_p := pf(SGPB_F_Stat, df1=1, df2=SGPB_ADF, lower.tail=FALSE)]
  smry_all[, SGPB_F_Simp := ((Mean_SGPB_Observed - Mean_SGPB_Imputed)^2)/((1 + 1/MM)*SGPB_B)]
  smry_all[, SGPB_F_p_simp := pf(SGP_F_Simp, df1=1, df2=nu, lower.tail=FALSE)]
  ...
  cols.to.keep <- c(institution.level, summary.level, grep("Mean_|SE_|Lambda|Gamma|RIV|_CI_|_F_p", names(smry_all), value=TRUE))
